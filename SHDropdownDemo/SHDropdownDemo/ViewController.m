...
//  SHDropdownDemo
//
//  Created by Nishadh Shrestha on 6/13/15.
...
@property (weak, nonatomic) IBOutlet SHDropdown *countryDropdown;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.countryDropdown.dropDownDelegate = self;
    ...
    for (id object in countries) {
        [[self countryDropdown] addItem: [[SHDropdownItem alloc] init:object tag:nil]];
    }
}

- (void) dropDown:(SHDropdown *)dropDown didSelectRow:(SHDropdownItem *)item row:(NSInteger)row {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Selected a country" message:item.title delegate:self cancelButtonTitle: nil otherButtonTitles:@"Ok", nil];
